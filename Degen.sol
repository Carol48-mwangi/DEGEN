...
contract DegenToken is ERC20, ERC20Burnable, Ownable {
    ...
    function redeemTokens(ItemType itemType, uint quantity) public {
        ...
        if (itemType == ItemType.Laptop) {
            tokenCost = 10 * 10 ** decimals();
        } else if (itemType == ItemType.GamingMouse) {
            tokenCost = 5 * 10 ** decimals();
        } else if (itemType == ItemType.Keyboard) {
            tokenCost = 7 * 10 ** decimals();
        } else if (itemType == ItemType.Headset) {
            tokenCost = 6 * 10 ** decimals();
        } else if (itemType == ItemType.ExternalHardDrive) {
            tokenCost = 8 * 10 ** decimals();
        } else {
